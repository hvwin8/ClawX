    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          <div class="config-sidebar__title">${translate('config.title')}</div>
        ...
        <div class="config-search">
          ...
          <input
            ...
            placeholder=${translate('config.search.placeholder')}
        ...
        <nav class="config-nav">
          ...
          >
            ...
            <span class="config-nav__label">${translate('config.sections.all')}</span>
          </button>
        ...
        <div class="config-sidebar__footer">
          <div class="config-mode-toggle">
            ...
            >
              ${translate('config.mode.form')}
            </button>
            <button
              class="config-mode-toggle__btn ${props.formMode === "raw" ? "active" : ""}
              @click=${() => props.onFormModeChange("raw")}
            >
              ${translate('config.mode.raw')}
            </button>
          </div>
        </div>
      </aside>
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
          <div class="config-actions__left">
            ${hasChanges ? html`
              <span class="config-changes-badge">${props.formMode === "raw" ? translate('config.status.unsavedChanges') : translate('config.status.unsavedChangeCount', { count: diff.length, plural: diff.length !== 1 ? 's' : '' })}</span>
            ` : html`
              <span class="config-status muted">${translate('config.status.noChanges')}</span>
            `}
          </div>
          <div class="config-actions__center">
            <div class="config-language-selector">
              <span>${translate('config.language.title')}:</span>
              <button 
                class="btn btn--sm ${localStorage.getItem('locale') === 'zh' ? 'active' : ''}"
                @click=${() => setLocale('zh')}
              >
                ${translate('config.language.zh')}
              </button>
              <button 
                class="btn btn--sm ${localStorage.getItem('locale') === 'en' ? 'active' : ''}"
                @click=${() => setLocale('en')}
              >
                ${translate('config.language.en')}
              </button>
            </div>
          </div>
          <div class="config-actions__right">
            <button class="btn btn--sm" ?disabled=${props.loading} @click=${props.onReload}>
              ${props.loading ? translate('config.actions.loading') : translate('config.actions.reload')}
            </button>
            <button
              ...
              @click=${props.onSave}
            >
              ${props.saving ? translate('config.actions.saving') : translate('config.actions.save')}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canApply}
              @click=${props.onApply}
            >
              ${props.applying ? translate('config.actions.applying') : translate('config.actions.apply')}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canUpdate}
            ...
            >
              ${props.updating ? translate('config.actions.updating') : translate('config.actions.update')}
            </button>
          </div>
        </div>
        ...
        ${hasChanges && props.formMode === "form" ? html`
          <details class="config-diff">
            <summary class="config-diff__summary">
              <span>${translate('config.diff.summary', { count: diff.length, plural: diff.length !== 1 ? 's' : '' })}</span>
            ...
            ? html`
                ${props.schemaLoading
                  ? html`<div class="config-loading">
                      ...
                      <span>${translate('config.status.loadingSchema')}</span>
                    </div>`
                  : renderConfigForm({
                ...
                ${formUnsafe
                  ? html`<div class="callout danger" style="margin-top: 12px;">
                      ${translate('config.warnings.formUnsafe')}
                    </div>`
                  : nothing}
              `
            : html`
                <label class="field config-raw-field">
                  <span>${translate('config.raw.label')}</span>
                  <textarea
                    .value=${props.raw}
                    @input=${(e: Event) =>
